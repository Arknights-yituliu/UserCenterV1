 ...
 *
 * <p>归属规则：uid 取自可信登录上下文，客户端即使携带 uid 也不作为归属依据；
 * 绑定关系按用户维度维护，干员数据按 ak_uid 只存一份并由全部绑定用户共享。</p>
 *
 * @author UserCenter
 */
public interface AkAccountService {
     ...
     */
    OperatorSaveResultVO saveOperators(Long uid, OperatorSaveRequest request);
}
