...
/**
 * 干员数据 Mapper（operator_progression_data）
 *
...
public interface OperatorProgressionDataMapper extends BaseMapper<OperatorProgressionData> {
     ...
     */
    @Select("SELECT * FROM operator_progression_data WHERE ak_uid = #{akUid} ORDER BY operator_id")
    List<OperatorProgressionData> selectByAkUid(@Param("akUid") String akUid);
     ...
     * @return 干员记录列表
     */
    @Select("<script>SELECT * FROM operator_progression_data WHERE ak_uid = #{akUid} AND operator_id IN "
            + "<foreach collection='operatorIds' item='item' open='(' separator=',' close=')'>#{item}</foreach>"
            + "</script>")
    List<OperatorProgressionData> selectByAkUidAndIds(@Param("akUid") String akUid,
     ...
     * @return 影响行数
     */
    @Insert("<script>INSERT INTO operator_progression_data "
            ...
            + "</foreach></script>")
    int batchInsert(@Param("items") List<OperatorProgressionData> items);
     ...
     * @return 影响行数
     */
    @Update("UPDATE operator_progression_data SET rarity = #{rarity}, level = #{level}, "
    ...
    int updateProgression(OperatorProgressionData record);
}
