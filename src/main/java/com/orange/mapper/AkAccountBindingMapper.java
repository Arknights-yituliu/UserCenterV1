...
public interface AkAccountBindingMapper {
    ...
    @Select("SELECT COUNT(1) FROM ak_account_binding "
            + "WHERE ak_uid = #{akUid} AND owner_uid = #{ownerUid} AND client_id = #{clientId}")
     ...
     * @return 影响行数
     */
    @Insert("INSERT INTO ak_account_binding (ak_uid, owner_uid, client_id) "
            + "VALUES (#{akUid}, #{ownerUid}, #{clientId})")
    int insertBinding(@Param("akUid") String akUid,
                      @Param("ownerUid") Long ownerUid,
                      @Param("clientId") String clientId);

    /**
     * 查询当前用户在指定客户端下已绑定的全部游戏账号 UID
     *
     * @param ownerUid 用户中心 UID
     * @param clientId 客户端标识
     * @return 游戏账号 UID 列表
    ...
    @Select("SELECT ak_uid FROM ak_account_binding "
            + "WHERE owner_uid = #{ownerUid} AND client_id = #{clientId} ORDER BY ak_uid")
